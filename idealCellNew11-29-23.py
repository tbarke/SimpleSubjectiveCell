...
def findAllocationAB(nA, nB, diff):
    ...
    ax.set_xlabel('A bound Receptor')
    ax.set_ylabel('B bound Receptor')
...
# print(MIsynEquiv[len(MIsynEquiv)-1])
# print(MIsynAdapt[len(MIsynAdapt)-1])
# print([a - b for a, b in zip(MIsynAdapt, MIsynEquiv)])
# exit()
...
"""
scatter = plt.scatter(MIAdapt, rewardsAdapt, label = "Adaptive Strategy", color = 'orange', alpha = 0.5, zorder = 2)
plt.scatter(MIEquiv, rewardsEquiv, label = "Equivalent Strategy", color = 'blue', alpha = 0.5, zorder = 2)
#plt.scatter(rewardsAdapt, syntacticTotAdapt, label = "Adaptive Strategy Syntactic", color = 'green', alpha = 0.5, zorder = 2)
#plt.scatter(rewardsEquiv, syntacticTotEquiv, label = "Equivalent Strategy Syntactic", color = 'purple', alpha = 0.5, zorder = 2)
#plt.scatter(2.5 - 2, 0, label = "Zero Information", color = 'red', edgecolor='black', s=100, zorder = 2)
#plt.scatter(0, 1.3365916687821622, label = "Maximum Information", color = 'black', edgecolor='black', s=100, zorder = 2)
plt.scatter(curMIAdapt, curReward - c1 - 100*c2, label = "Adaptive Strategy (100 receptors)", color='orange', edgecolor='black', s=100, zorder = 2)
plt.scatter(curMIEquiv, curRewardequiv - c1 - 100*c2, label = "Equivalent Strategy (100 receptors)",  color='blue', edgecolor='black', s=100, zorder = 2)
cbar = plt.colorbar(scatter, label='Receptors (Log Scale, Base 10)')
value_ranges = ["2", "600", "1300"]  # Adjust these based on your data
colors = [scatter.cmap(scatter.norm(value)) for value in (math.log10(2), math.log10(600), math.log10(1300))]  # Replace with actual values or percentiles
# Create patches for the legend
patches = [mpatches.Patch(color=colors[i], label=value_ranges[i]) for i in range(len(value_ranges))]
plt.legend(handles=patches)
plt.ylabel("Growth")
plt.xlabel("$E(I(A_{ext}, B_{ext}; move)|_{\Delta_t})$")
plt.grid(zorder = 1)
#plt.legend()
plt.show()
"""
...
plt.clf()
print(rewardPerReceptor[0])

print(recList)
print(rewardPerReceptor)
scatter = plt.scatter(recList, rewardPerReceptor, label="SI", zorder=2)
# plt.scatter((curMIAdapt - curMIEquiv), (curReward - c1 - 100*c2) - (curRewardequiv - c1 - 100*c2), label = "100 Receptors", zorder = 2)
# plt.scatter(rewardsAdapt, [a - b for a, b in zip(MIAdapt, MIEquiv)], label = "SI (100 receptors)", zorder = 2)
# plt.plot(rewardsEquiv, MIsynEquiv, label = "Equivalent Strategy")
# cbar = plt.colorbar(scatter, label='Receptors (Log Scale, Base 10)')
# value_ranges = ["2", "600", "1300"]  # Adjust these based on your data
# colors = [scatter.cmap(scatter.norm(value)) for value in (2, 600, 1300)]  # Replace with actual values or percentiles
# patches = [mpatches.Patch(color=colors[i], label=value_ranges[i]) for i in range(len(value_ranges))]
# plt.legend(handles=patches)
# plt.ylabel("$\hat{R}_{A} - \hat{R}_{E}$", rotation = 0)
...
exit()
plt.clf()
plt.scatter([a - b for a, b in zip(MIAdapt, MIEquiv)], rewardsAdapt, label="SI", cmap='viridis', zorder=2)
plt.scatter((curMIAdapt - curMIEquiv), curReward - c1 - 100 * c2, label="SI 100", zorder=2)
# plt.scatter(rewardsAdapt, [a - b for a, b in zip(MIAdapt, MIEquiv)], label = "SI (100 receptors)", zorder = 2)
# plt.plot(rewardsEquiv, MIsynEquiv, label = "Equivalent Strategy")
cbar = plt.colorbar(scatter, label='Receptors (Log Scale, Base 10)')
value_ranges = ["2", "600", "1300"]  # Adjust these based on your data
colors = [scatter.cmap(scatter.norm(value)) for value in
          (math.log10(2), math.log10(600), math.log10(1300))]  # Replace with actual values or percentiles
# Create patches for the legend
patches = [mpatches.Patch(color=colors[i], label=value_ranges[i]) for i in range(len(value_ranges))]
plt.legend(handles=patches)
plt.ylabel("Growth")
...
print("this one")
plt.clf()
scatter = plt.scatter([a - b for a, b in zip(MIsynAdapt, MIsynEquiv)],
                      [a - b for a, b in zip(rewardsAdapt, rewardsEquiv)], c=recList, cmap='viridis', zorder=2)
# plt.scatter((curMIsynAdapt - curMIsynEquiv), (curReward - c1 - 100*c2) - (curRewardequiv - c1 - 100*c2), label = "100 receptors", zorder = 2)
# plt.scatter(rewardsAdapt, [a - b for a, b in zip(MIAdapt, MIEquiv)], label = "SI (100 receptors)", zorder = 2)
# plt.plot(rewardsEquiv, MIsynEquiv, label = "Equivalent Strategy")
cbar = plt.colorbar(scatter, label='Receptors (Log Scale, Base 10)')
value_ranges = ["2", "600", "1300"]  # Adjust these based on your data
...
patches = [mpatches.Patch(color=colors[i], label=value_ranges[i]) for i in range(len(value_ranges))]
plt.legend(handles=patches)
plt.ylabel("$\hat{R}_{A} - \hat{R}_{E}$")
plt.xlabel("$I_{subj}$")
plt.grid(zorder=1)
plt.legend(bbox_to_anchor=(5, 1))
plt.show()
...
plt.clf()
plt.scatter([a - b for a, b in zip(MIsynAdapt, MIsynEquiv)], recList, zorder=2)
# plt.scatter((curMIsynAdapt - curMIsynEquiv), curReward - c1 - 100*c2 , label = "100 Receptors", zorder = 2)
# plt.scatter(rewardsAdapt, [a - b for a, b in zip(MIAdapt, MIEquiv)], label = "SI (100 receptors)", zorder = 2)
# plt.plot(rewardsEquiv, MIsynEquiv, label = "Equivalent Strategy")
plt.ylabel("Growth")
plt.xlabel("$Strict Information Difference$")
plt.grid(zorder=1)
plt.legend()
plt.show()
...
# plt.rcParams['text.usetex'] = True
slopes = []
for i in range(len(MIDiff) - 1):
    slopes.append((rewardDiff[i + 1] - rewardDiff[i]) / (MIDiff[i + 1] - MIDiff[i]))

plt.clf()
print(slopes[0])
plt.scatter(recList[1:], slopes, zorder=2)
# plt.scatter((curMIsynAdapt - curMIsynEquiv), curReward - c1 - 100*c2 , label = "100 Receptors", zorder = 2)
# plt.scatter(rewardsAdapt, [a - b for a, b in zip(MIAdapt, MIEquiv)], label = "SI (100 receptors)", zorder = 2)
# plt.plot(rewardsEquiv, MIsynEquiv, label = "Equivalent Strategy")
plt.ylabel(r"$\frac{\Delta (\hat{R}_{A} - \hat{R}_{E})}{\Delta I_{subj}}$", rotation=0, fontsize=15)
